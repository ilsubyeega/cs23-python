...
with open('Week 14/identifiedacc_bin_dump.tmp', 'wb') as io_read:
    io_read.write(f'{123:10d}{1000}'.encode())

with open('Week 14/identifiedacc_bin_dump.tmp', 'rb') as io_read:
    raw = io_read.read()
    # dont know about byte length of last payload.
    # possibly parsable after decoding to string.
    acc = IdentifiedAccount(id=int(raw[:10].decode().strip()), balance=int(raw[10:].decode().strip()))
